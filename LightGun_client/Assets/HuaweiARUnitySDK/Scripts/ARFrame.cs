...
namespace HuaweiARUnitySDK
{
    ...
    public class ARFrame
    {
        ...
        public static void GetTrackables<T>(List<T> trackableList, ARTrackableQueryFilter filter) where T : ARTrackable
        {
            ...
            {
                ARDebug.LogWarning("GetTrackables test trackableList == null");
            ...
            {
                trackableList.Clear();
                ARDebug.LogWarning("GetTrackables test SessionStatus != ARSessionStatus.RUNNING");
                return;
            }
        ...
        /**
         * \if english
         * @brief Gets the unrotated and uncropped intrinsics for the image (CPU) stream.
         * @exception ARNotYetAvailableException \copybrief ARNotYetAvailableException
         * \else
         * @brief 获取图像（CPU）流未旋转和未裁剪的内参。
         * @exception ARNotYetAvailableException \copybrief ARNotYetAvailableException
         * \endif
         */
        public static ARCameraIntrinsics ImageIntrinsics
        {
            get
            {
                var cameraPtr = ARSessionManager.Instance.m_ndkSession.FrameAdapter.AcquireCameraHandle();
                if (cameraPtr == null)
                {
                    ARDebug.LogWarning("ARFrame cameraPtr is null!");
                    return new ARCameraIntrinsics();
                }
                ARCameraIntrinsics result = ARSessionManager.Instance.m_ndkSession.CameraAdapter.GetImageIntrinsics(cameraPtr);
                ARSessionManager.Instance.m_ndkSession.CameraAdapter.Release(cameraPtr);
                return result;
            }
        }
         ...
         * @exception ARNotYetAvailableException \copybrief ARNotYetAvailableException
         */
        public static ARCameraImageBytes AcquirPreviewImageBytes()
        {
            if (ARSessionManager.Instance.SessionStatus != ARSessionStatus.RUNNING &&
                ARSessionManager.Instance.SessionStatus != ARSessionStatus.PAUSED)
            {
